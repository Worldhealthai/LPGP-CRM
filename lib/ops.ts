...
// A separate secret on purpose — a leaked read key must never be able to
// create a financial record in the tracker. Absent means reads only.
const WRITE_KEY = process.env.OPS_BRIDGE_WRITE_KEY ?? "";
const TIMEOUT_MS = 8000;
// Invoice uploads are base64, so writes get longer than a read's budget.
...
/** True when this CRM is allowed to record deals in the ops panel. */
export function isOpsWriteEnabled(): boolean {
  return Boolean(BASE && KEY && WRITE_KEY);
}
...
/**
 * Write to the ops panel. Same tagged-result contract as reads — a failed
 * write must surface as a message, never an unhandled throw mid-form.
 */
async function opsWrite<T>(
  path: string,
  body: unknown,
  ...
  if (!WRITE_KEY) {
    return {
      ...
      reason: "not_configured",
      error:
        "Recording deals is off. Set OPS_BRIDGE_WRITE_KEY here and on the tracker to enable it.",
    };
  }
  ...
  try {
    ...
    // The bridge explains its refusals; pass those through verbatim rather
    // than flattening them into "something went wrong".
    if (!res.ok) {
      let message = `Ops panel returned ${res.status}.`;
      try {
      ...
      const reason: OpsFailure =
        res.status === 401 ? "unauthorized" : res.status === 503 ? "not_configured" : "bad_response";
    ...
    return {
      ...
      error: /timeout|abort/i.test(msg)
        ? "The ops panel did not respond in time. The deal may not have been saved — check the tracker before retrying."
        : `Could not reach the ops panel: ${msg}`,
    };
  }
}

export type OpsDealInput = {
  company: string;
  title?: string;
  contact_name?: string;
  ...
  paid_inc_vat?: number | string | null;
  tax_vat?: number | string | null;
  bank?: string;
  initials?: string;
  deal_month?: string;
  ...
  event_packages?: { event_id: number; amount: number | string; package_label?: string }[];
};

export async function createOpsDeal(input: OpsDealInput): Promise<OpsResult<OpsDeal>> {
  return opsWrite<OpsDeal>("/api/bridge/deals", input);
}

export async function updateOpsDeal(
  dealId: number,
  input: OpsDealInput,
): Promise<OpsResult<OpsDeal>> {
  return opsWrite<OpsDeal>(`/api/bridge/deals/${dealId}`, input, "PATCH");
}

export async function attachOpsInvoice(
  dealId: number,
  slot: 1 | 2,
  file: { name: string; data: string },
): Promise<OpsResult<{ ok: true; name: string }>> {
  return opsWrite(`/api/bridge/deals/${dealId}/invoice/${slot}`, file);
}
